        ...
        <div className="grid grid-cols-1 lg:grid-cols-[60%_auto] md:grid-cols-1 items-center md:gap-12 gap-10">
          ...
          <div className="work-with-us-features grid grid-cols-1 md:grid-cols-1 xl:gap-10 md:gap-6 gap-6">
            {features.map((feature, index) => (
              <div
                key={feature.id}
                className={`md:p-0 relative md:rounded-none rounded-[10px] md:bg-transparent transition-all duration-300 ease-in-out overflow-hidden
                  ${isMobile ? `${feature.mobileBg} ${index === activeIndex ? 'py-[20px] px-[20px]' : 'py-[20px] px-[20px]'}` : ''}
                `}
                onClick={() => handleClick(index)}
              >
                <div
                  className={`flex md:space-x-4 space-x-3 md:mb-4 flex-row gap-0 text-left justify-start cursor-pointer`}
                >
                  <Image
                    src={feature.icon}
                    alt={feature.title}
                  ...
                  <span className='md:hidden block arrow-icon'>
                    <svg
                      ...
                      viewBox="0 0 24 24"
                      fill="none"
                      className={`trasition-all duration-300 ease-in-out ${isMobile ? index === activeIndex ? 'rotate-180' : 'rotate-0' : ''}`}
                    //   ${
                    //   isMobile
                    //     ? index === activeIndex
                ...
                >
                  <ul
                    className={`lg:pl-[57px] md:pl-[41px] pl-[36px] text-title md:mt-0 mt-4
                    ${isMobile ? (index === activeIndex ? 'block' : 'hidden') : 'block'}`}
                  >
                    {feature.items.map((item, itemIndex) => (
                      <li
                        key={itemIndex}
                        className="md:leading-[1] leading-[normal] font-roboto text-[16px] md:mb-3 mb-2"
                      >
                        {item}
                      </li>
                    ))}
